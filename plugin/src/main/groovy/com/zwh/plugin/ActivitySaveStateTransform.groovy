...
//                { it.name == "onCreate" && it.parameterTypes == [bundleCtClass] as CtClass[] }
        CtMethod restoreCtMethod = ctClass.declaredMethods.find
                { it.name == "initData" && it.parameterTypes == [bundleCtClass] as CtClass[] }
        ...
        ctClass.declaredFields.each { field ->
            for (Annotation mAnnotation : field.getAnnotations()) {
                if (mAnnotation.annotationType().canonicalName == Constant.AUTO_SAVE_STATE_ANNOTATION){
                    list.add(field)
                }
            }
        }
