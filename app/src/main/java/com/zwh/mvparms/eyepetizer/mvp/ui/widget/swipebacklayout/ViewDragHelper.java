...
public class ViewDragHelper {
    ...
    private int getEdgeTouched(int x, int y) {
        int result = 0;
        if ((x < mParentView.getLeft() + mEdgeSize)&&mTrackingEdges ==EDGE_LEFT)
            result = EDGE_LEFT;
        if ((y < mParentView.getTop() + mEdgeSize)&&mTrackingEdges ==EDGE_TOP)
            result = EDGE_TOP;
        if ((x > mParentView.getRight() - mEdgeSize)&&mTrackingEdges ==EDGE_RIGHT)
            result = EDGE_RIGHT;
        if ((y > mParentView.getBottom() - mEdgeSize)&&mTrackingEdges ==EDGE_BOTTOM)
            result = EDGE_BOTTOM;
